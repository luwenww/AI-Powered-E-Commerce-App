...
class Product(models.Model):
    ...
    unit_price = models.DecimalField(
        ...
        validators=[
            MinValueValidator(
                0.009, message=("Ensure this value is greater than or equal to 0.01.")
            )
        ],
    )
...
class Customer(models.Model):
    ...
    )  # referring to the customeradmin list_display
    def first_name(self):
...
class Order(models.Model):
    ...
    PAYMENT_STATUS_SHIPPED = "S"
    PAYMENT_STATUS_CHOICES = [
        (PAYMENT_STATUS_PENDING, "Pending"),
        (PAYMENT_STATUS_COMPLETE, "Complete"),
        (PAYMENT_STATUS_FAILED, "Failed"),
        (PAYMENT_STATUS_RECEIVED, "Payment Received"),
...
class Review(models.Model):
    ...
    rating = models.DecimalField(
        max_digits=2, decimal_places=1, choices=RATINGS_CHOICES
    )
